...
class KarcherHome:
    ...
    async def _process_response(self, resp: aiohttp.ClientResponse, prop=None) -> Any:
        if resp.status != 200:
            resp.close()
            raise KarcherHomeException(-1, "HTTP error: " + str(resp.status))
    ...
    def _mqtt_connect(self, wait_for_connect=False):
        if self._session is None or self._session.mqtt_token == "" or self._session.user_id == "":
        ...
        self._mqtt = MqttClient(
            host=u.hostname, port=u.port, username=self._session.user_id, password=self._session.mqtt_token
        )
    ...
    def login_token(self, auth_token: str, mqtt_token: str, register_id=None) -> Session:
    ...
    async def logout(self):
        ...
        await self._process_response(await self._request("POST", "/user-center/auth/logout"))
        self._session = None
    ...
    async def get_user_info(self) -> UserProfile:
        ...
        resp = await self._request("GET", "/user-center/app/user/profile")
        data = await self._process_response(resp)
    ...
    async def get_devices(self) -> List[Device]:
        ...
        resp = await self._request(
            "GET", "/smart-home-service/smartHome/user/getDeviceInfoByUserId/" + self._session.user_id
        )
    ...
    async def get_map_data(self, dev: Device, map: int = 1):
        ...
        mapDir = (
            TENANT_ID
            + "/"
            + dev.product_mode_code
            + "/"
            + dev.sn
            + "/01-01-2022/map/temp/0046690461_"
            + dev.sn
            + "_"
            + str(map)
        )
    ...
    def get_device_topics(self, dev: Device) -> List[str]:
        return get_device_topics(dev.product_id, dev.sn)
    ...
    def get_device_properties(self, dev: Device) -> DeviceProperties:
        ...
        self._wait_for_topic(get_device_topic_property_get_reply(dev.product_id, dev.sn))
